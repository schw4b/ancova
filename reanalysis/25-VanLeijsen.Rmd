...
# PATH = file.path(path.expand("~"), "results", "data", "ancova") # ancova project folder
# data = read_excel(file.path("..", "dataPrimaryStudies", "25-VanLeijsen", "25-VanLeijsen-1.xlsx"))

data = read_excel("../results/data/25-VanLeijsen/VanLeijsen-1.xlsx")

mod <- lm(AB38 ~ age + hypertension, data= data[data$mb_presence_b == "1",])
summary(mod)

mod <- lm(AB38 ~ age + hypertension, data= data[data$mb_presence_b == "0",])
summary(mod)

mod <- lm(AB38 ~ age + hypertension, data= data[data$lac_presence_b == "1",])
summary(mod)

mod <- lm(AB38 ~ age + hypertension, data= data[data$lac_presence_b == "0",])
summary(mod)

mod <- lm(tbv_b ~ age + hypertension + AB38, data= data)
summary(mod)

anov <- aov(AB38 ~ age + hypertension, data= data[data$mb_presence_b == "1",])
anov

anov <- aov(AB38 ~ age + hypertension, data= data[data$mb_presence_b == "0",])
anov

anov <- anov(AB38 ~ age + hypertension, data= data[data$mb_presence_b == "1",])
anov
data0 = gather(data, key = "IVlevel", "IVvalue", mb_presence_b, lac_presence_b)

data$IV = NA

data$IVValue[data$mb_presence_b == "1",]<- "microbleed"

data[data$mb_presence_b == "1",] == data$IV
dim(data)
data$group.factor = NA

if (group.factor) {
  if (data$IVlevel == "mb_presence_b" & ) {
    data$group.factor <- "microbleeds"
  } else {
     data$group.factor <- "no_mic"
  }
} else {
  if (group.factor) {
    if (data$IVlevel == "lac_presence_b" & data$IVvalue == "1") <- "bottomright"
  } else {
    data$group.factor <- "lac"
  } else {
    data$group.factor <- "no_mic"
  }
} 

data$group.factor <- ifelse(data$IVlevel == "mb_presence_b" & data$IVvalue == "1", "microbleed", "no_microbleed")
data$group.factor <- ifelse(data$IVlevel == "lac_presence_b" & data$IVvalue == "1","lac", "no_lac")
```
...
# Reproducing Table 2 
```{r, include = TRUE, echo = FALSE, eval = TRUE, tidy = TRUE}
lm_lac = lm(AB38 ~ sex + age, data = data)
summary(lm_lac)



#contrasts(data$IV)<-cbind(c(-2,1,1), c(0,-1,1))
ancovaModel <-aov(AB38 ~ age + sex, 
                  data = data)
ancova <- aov(AB38 ~ mb_presence_b, data = data)
ancova
