...
---
The following reanalysis comes from the paper at doi.org : 10.1161/STROKEAHA.117.019810
...
The outcome of interest in the ANCOVA is "SVD"
The independant variables are plasma Aβ (two levels : Aβ38 and Aβ40)
...
```{r, include = TRUE, echo = FALSE, eval = TRUE, tidy = TRUE}
data = read_excel("../data/25-VanLeijsen/VanLeijstudyNo25DATA.xlsx")
names(data)
DV = mb_presence_b #lac_presence_b is also an SVD marker which is a DV
...
ancovaModel<-aov(DV ~ ageCOV + ab38IV + ab10IV + ab42IV, data = data)
lm_ancova <- lm(DV ~ COV + ageCOV + ab38IV + ab10IV + ab42IV, data = data)
summary.lm(ancovaModel)
Anova(ancovaModel, type="I") # this is not a balanced study
...
mod1 <- aov(DV ~ ab38IV, data = data)
mod2 <- aov(DV ~ ab40IV, data = data)
mod3 <- aov(DV ~ ab42IV, data = data)
