...
data$Group.factor = factor(data$Group.factor,
                           levels = c("CR", "DHR",
                                      "CON", "LUN", "RES"), order = FALSE)
data$Week <- factor(data$Week, levels = c("E1", "E2", "E3", "E4", "E5", "E6", "E7", "E8"), ordered = TRUE)
data$time[data$Week == "E1"] = 1
data$time[data$Week == "E2"] = 2
data$time[data$Week == "E3"] = 3
data$time[data$Week == "E4"] = 4
data$time[data$Week == "E5"] = 5
data$time[data$Week == "E6"] = 6
data$time[data$Week == "E7"] = 7
data$time[data$Week == "E8"] = 8
```
...
tab.dv = array(NA, dim=c(5,3))
tab.dv[,1] = levels(data$Group.factor)
tab.dv[,2] = summary(na.omit(data$Group.factor))
tab.dv[,3] = tapply(data$Weight, data$Group.factor, 
...
ggplot(na.omit(data), aes(y=Weight, x=Group.factor, color=Group.factor)) +
  ...
  xlab("group") + ylab("weight") + ggtitle("Independant variable")
```

## Covariate(s)
```{r}
idx = c(1, 3, 2, 4) # sorting as in publication
tab.cv = array(NA, dim=c(5,3))
tab.cv[,1] = levels(data$Group.factor)
tab.cv[,2] = summary(na.omit(data$Group.factor))
tab.cv[,3] = tapply(data$Weight, data$Group.factor, 
                  function (x) sprintf("%0.1f (%0.1f)", mean(x, na.rm = TRUE), sd(x, na.rm = TRUE)))
...
contrasts(data$Group) = contr.helmert(5) 

fit.ancova = aov(Weight ~ Week + Group.factor, data = data)
...
There is evidence of significance that the assumption of homogeneity of variance hold across all groups. The standard deviation of weights across groups are similar numerically as well.
```{r}
tapply(data$Weight, data$Group.factor, sd, na.rm = TRUE)
leveneTest(Weight ~ Group.factor, data = data)
```
...
There is evidence of significance that the covairate is not independant to the independant variable.
```{r}
fit.cv = aov(Weight ~ Group.factor, data = data) 
summary(fit.cv)
```
...
There is no interaction with time and group factor. However, there is evidence of significance that each group has a different effect 

In a visual inspection, each group has a slightly dissimilar slope with respect to time. 
```{r}
fit.hrs = aov(Weight ~ Week*Group.factor, data = data)
Anova(fit.hrs, type=3)
```

```{r fig.height=2, fig.width=3.5}
ggplot(data, aes(y=Weight, x=time, color=Group.factor, shape=Group.factor)) +
  geom_point() +
