...
EmotionIV <- c(rep("Neutral", 585), rep("Negative", 585), rep("Positive", 585))
IntensityIV <- c(rep("One", 351), rep("Two", 351), rep("Three", 351), rep("Four", 351), rep("Five", 351))
ElectrodeIV <- c(rep("ElectrodeOne", 585), rep("ElectrodeTwo", 585), rep("ElectrodeThree", 585))
ID <- as.data.frame(rep(data$Subject, 3))
colnames(ID) -> "ID"

COV <- rep(data$RST_BIS, 3) 
...
DV0 <- gather(data, "DVcat", "DV", -Subject, -Oss) 
DV0 <- DV0[,3:4]
names(DV0)
dim(DV0)
View(DV0)
data <- as.data.frame(c(ID, COV, EmotionIV, IntensityIV, ElectrodeIV, DV0))
View(data)

for (i in 3:48) {
  DV[1:1775,i] <- rbind(c(data[1:39, i], data[1:39, i+1]))
  return(DV)
}
DV

DV <- rbind(c(data[1:39, 3], data[1:39, 3+1]))
